-- Robust Database Migration for LKRM Player Management System
-- This migration adapts to existing table structures and handles all edge cases
...
-- Step 7: Handle player_notes table - check existing structure and adapt
DO $$
DECLARE
    table_exists BOOLEAN;
    column_exists BOOLEAN;
BEGIN
    -- Check if player_notes table exists
    SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_schema = 'public' 
        AND table_name = 'player_notes'
    ) INTO table_exists;
    
    IF NOT table_exists THEN
        RAISE NOTICE 'Creating player_notes table...';
    ...
    ELSE
        RAISE NOTICE 'player_notes table already exists, checking structure...';
        
        -- Check if it has the right columns, add missing ones
        SELECT EXISTS (
            SELECT FROM information_schema.columns 
            WHERE table_schema = 'public' 
            AND table_name = 'player_notes' 
        ...
        IF NOT column_exists THEN
            RAISE NOTICE 'Adding player_id column to existing player_notes table...';
            ALTER TABLE player_notes ADD COLUMN player_id BIGINT;
            
            -- Try to add foreign key constraint
            BEGIN
                ALTER TABLE player_notes ADD CONSTRAINT fk_player_notes_player_id 
        ...
        ALTER TABLE player_notes ADD COLUMN IF NOT EXISTS note_text TEXT;
        ALTER TABLE player_notes ADD COLUMN IF NOT EXISTS user_id UUID;
        ALTER TABLE player_notes ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;

-- Step 8: Handle player_goals table - check existing structure and adapt
DO $$
DECLARE
    table_exists BOOLEAN;
    column_exists BOOLEAN;
BEGIN
    ...
    SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_schema = 'public' 
        AND table_name = 'player_goals'
    ) INTO table_exists;
    
    IF NOT table_exists THEN
        RAISE NOTICE 'Creating player_goals table...';
    ...
    ELSE
        RAISE NOTICE 'player_goals table already exists, checking structure...';
        ...
        IF NOT column_exists THEN
            RAISE NOTICE 'Adding player_id column to existing player_goals table...';
            ALTER TABLE player_goals ADD COLUMN player_id BIGINT;
            
            -- Try to add foreign key constraint
            BEGIN
                ALTER TABLE player_goals ADD CONSTRAINT fk_player_goals_player_id 
                    FOREIGN KEY (player_id) REFERENCES players(id) ON DELETE CASCADE;
            EXCEPTION WHEN OTHERS THEN
                RAISE NOTICE 'Could not add foreign key constraint: %', SQLERRM;
            END;
        ...
        -- Add other missing columns
        ALTER TABLE player_goals ADD COLUMN IF NOT EXISTS goal_text TEXT;
        ALTER TABLE player_goals ADD COLUMN IF NOT EXISTS user_id UUID;
        ALTER TABLE player_goals ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;
...
-- Step 10: Create indexes for notes and goals (only if columns exist)
DO $$
BEGIN
    -- Check if player_id column exists in player_notes before creating index
    IF EXISTS (
        SELECT FROM information_schema.columns 
        WHERE table_schema = 'public' 
        AND table_name = 'player_notes' 
...
-- Similar policies for notes and goals (only if player_id column exists)
DO $$
BEGIN
    IF EXISTS (
        SELECT FROM information_schema.columns 
...
SELECT 
    ...
    p.last_name,
    p.school_year,
    p."positionId" as position_id,  -- Use the actual column name from your database
...
FROM players p
LEFT JOIN positions pos ON p."positionId" = pos.id  -- Use the actual column name
...
DO $$
BEGIN
    RAISE NOTICE 'Robust migration completed successfully!';
    ...
    RAISE NOTICE 'Notes and goals tables have been created or updated as needed.';
