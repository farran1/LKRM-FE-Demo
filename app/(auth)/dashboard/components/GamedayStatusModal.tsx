...
import EditTask from '../../tasks/components/edit-task';
...
export default function GamedayStatusModal({ 
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      console.log('GamedayStatusModal task response:', res?.data);
      ...
      if ((res as any)?.data?.data) {
        ...
        const sanitizedTasks = (res as any).data.data.map((task: any) => ({
          ...
          // Add fields expected by TaskDetailModal
  ...
  const handleTaskRefresh = () => {
    // Refresh the tasks list when a task is updated
    if (isOpen && statusId && eventId) {
      fetchTasks();
    }
  };
      ...
      render: (record: any) => {
        try {
          ...
          return (
            <Tag 
              style={{ 
                backgroundColor: record.task_priorities.name === 'High' ? '#ff6b6b' : 
                               record.task_priorities.name === 'Medium' ? '#ffd93d' : '#4db8ff',
                color: '#fff',
                border: 'none',
                fontWeight: 500,
                fontSize: '12px'
  ...
  return (
    <Modal
      ...
      className="gameday-status-modal"
      styles={{
        mask: {
