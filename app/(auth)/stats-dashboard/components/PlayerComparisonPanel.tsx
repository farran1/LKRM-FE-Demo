...
const PlayerComparisonPanel: React.FC<PlayerComparisonPanelProps> = ({ isFocused = false, onToggleFocus }) => {
  ...
  // Find top performers (only consider players with stats > 0)
  const playersWithPoints = playerStats.filter(p => (p.avgPoints || 0) > 0);
  const playersWithRebounds = playerStats.filter(p => (p.avgRebounds || 0) > 0);
  const playersWithAssists = playerStats.filter(p => (p.avgAssists || 0) > 0);
  
  const topScorer = playersWithPoints.length > 0 ? playersWithPoints.reduce((top, current) => 
    (current.avgPoints || 0) > (top.avgPoints || 0) ? current : top, playersWithPoints[0]) : null;
  const topRebounder = playersWithRebounds.length > 0 ? playersWithRebounds.reduce((top, current) => 
    (current.avgRebounds || 0) > (top.avgRebounds || 0) ? current : top, playersWithRebounds[0]) : null;
  const topAssister = playersWithAssists.length > 0 ? playersWithAssists.reduce((top, current) => 
    (current.avgAssists || 0) > (top.avgAssists || 0) ? current : top, playersWithAssists[0]) : null;
